...
(function() {
  ...
  if (!doc.getElementById('spider-style')) {
    ...
    st.textContent = `
      ...
      #egg-arrow {
        position:fixed; top:55px; right:70px;
        color:#13D842;
  ...
  // Reset on next nav click after burst
  ...
  function burstEgg() {
    ...
    for (var i = 0; i < 25; i++) {
      (function() {
        ...
        setTimeout(function() { makeSpider(cx, cy); }, delay);
      })();
    }
  }
  ...
  function makeSpider(x, y) {
    ...
    var life = 0;
    var maxLife = 100 + Math.random() * 100;
    function move() {
